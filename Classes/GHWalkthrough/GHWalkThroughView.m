...
- (void)orientFooter
{
    ...
    else
    {
        ...
        if (isRotated) {
            ...
            self.skipButton.frame = CGRectMake(self.frame.size.width - 120, 70., 120, 30);
...
- (void)buildFooterView
{
    ...
    self.pageControl.pageIndicatorTintColor = [UIColor colorWithRed:180.0f/255.0f green:180.0f/255.0f blue:180.0f/255.0f alpha:1.0f];
    self.pageControl.currentPageIndicatorTintColor = [UIColor colorWithRed:208.0f/255.0f green:39.0f/255.0f blue:46.0f/255.0f alpha:1.0f];
//    [self addSubview:self.pageControl];
    [self bringSubviewToFront:self.pageControl];
    
    self.skipButton = [UIButton buttonWithType:UIButtonTypeCustom];
    self.skipButton.titleLabel.font = [UIFont fontWithName:FONTSTYLE size:14.0];
    
    self.skipButton.frame = CGRectMake(self.frame.size.width - 80, self.pageControl.frame.origin.y - ((30 - self.pageControl.frame.size.height)/2), 80, 10);
    //self.skipButton.frame = CGRectMake(80, self.pageControl.frame.origin.y - ((30 - self.pageControl.frame.size.height)/2), 80, 10);
...
- (void) scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    for (UICollectionViewCell *cell in [self.collectionView visibleCells]) {
        ...
        if (indexPath.row == (int)([self.dataSource numberOfPages] - 1)) {
            [self.skipButton setTitle:@"Fooze Away!" forState:UIControlStateNormal];
